    ...
    <div class="commonright container">
            <div class="filter">
                <div class="filter-item">
                    <label class="filter-label">第36页</label>
...
export default {
    ...
    methods:{
        ...
        getData(){
            ...
            mgconfs.tangpayConfList(ptdata).then(res=>{
                if(res.ok){
                    let data=res.data;
                    if(data){
                        this.data.list=data;
        ...
        formSubmit(){
            this.$refs['form'].validate(async (valid) => {
                if (valid) {
                    ...
                    /* 新增 */
                    if(type=="add"){
                        res=await mgconfs.tangpayConfAdd(postdata);
                    }
                    /* 修改 */
                    if(type=="edit"){
                        res=await mgconfs.tangpayConfUpdate(postdata);
                    }
                    if(res.ok){
                        this.dialog.visible=false;
                        successTips("保存成功");
