    ...
    <div class="commonright container">
        ...
        <div class="contentpad">
            <div :class="['menulist', dragover ? '' : 'menuhover']" id="menulist">
                ...
                <MenuItem :menusItem="listData" ref="menuparent" :applyThis="applyThis">
                </MenuItem>
            </div>
        </div>
        <!-- 删除 -->
            ...
            <el-form :model="dialog.form" :rules="dialog.rules" :label-width="$protovar.fmlabwidth" ref="form"
                ...
                <div class="tmcol">
                    <el-form-item label="上级菜单" prop="pid">
                        <ElTreeSelect popoverClass="fas" v-model="dialog.form.pid" :treeParams="treeParams"
                            :selectParams="selectParams" ref="treeSelect"></ElTreeSelect>
                    </el-form-item>
                </div>
                <div class="tmcol">
                ...
                <div class="tmcol">
                    <el-form-item label="状态" prop="status">
                        <el-select v-model="dialog.form.status">
                            <el-option v-for="item in statuselect" :key="item.id" :value="item.id"
                                :label="item.label"></el-option>
                        </el-select>
                    </el-form-item>
                </div>
        ...
        <!-- 禁用二次确认弹框 -->
        <DisableOper :show="disableoper.show" @handleDisableClose="handleDisableShow" @sureDisableOper="sureDisableOper">
        </DisableOper>
    </div>
</template>
<script>
...
import { mgauth2, msgTips, successTips } from '@_/axios/path';
...
export default {
    ...
    methods: {
        ...
        getData(item) {
            this.listData = [];
            mgauth2.mgAuthList(this.filters).then(res => {
                if (res.ok) {
                    let data = res.data;
                    if (data) {
                        this.listData = data;
        ...
        saveSort() {
            ...
            mgauth2.mgAuthIndex(ptdata).then(res => {
                if (res.ok) {
                    this.getData();
                } else {
                    this.getData();
        ...
        sureDel() {
            ...
            mgauth2.mgAuthDel(ptdata).then(res => {
            ...
            this.$refs.form.validate(async (valid) => {
                if (valid) {
                    ...
                    if (this.dialog.type == "add" || this.dialog.type == "addchild") {
                        res = await mgauth2.mgAuthAdd(postdata);
                    }
                    if (this.dialog.type == "edit") {
                        res = await mgauth2.mgAuthUpdate(postdata);
                    }
                    if (res.ok) {
        ...
        comDisable(item) {
            ...
            mgauth2.mgAuthDisable(ptdata).then(res => {
                if (res.ok) {
                    successTips("修改成功");
                    this.getData();
                } else {
