...
<script>
import {mgconf1,msgTips,successTips} from '@_/axios/path';
export default {
    data(){
        return {
            filters:{
                searchkey:'',
                searchkey1:'wx.wallet.withhold',
                type:''
            },
            data:{//数据表格
    ...
    methods:{
        ...
        getData(){
            ...
            mgconf1.mgConfList(ptdata).then(res=>{
                if(res.ok){
                    let data=res.data;
                    if(data){
                        this.data.list=data;
        ...
        formSubmit(){
            this.$refs['form'].validate(async (valid) => {
                if (valid) {
                    ...
                    /* 新增 */
                    if(type=="add"){
                        res=await mgconf1.mgConfAdd(postdata);
                    }
                    /* 修改 */
                    if(type=="edit"){
                        res=await mgconf1.mgConfUpdate(postdata);
                    }
                    if(res.ok){
                        this.dialog.visible=false;
                        successTips("保存成功");
