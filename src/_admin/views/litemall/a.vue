<template>
    <div class="conmonright container">
        ...
        <!--        添加页面-->
        <el-dialog :visible.sync="dialog.visible" width="70%" class="tmdialog" :modal="false">
            <div>
                <!-- 卡片试图-->
                <el-card style="overflow:auto;height:700px">
                    <!-- 提示信息 closable=>不能关闭的-->
                        ...
                        <el-tabs tab-position="left">
                            <el-tab-pane label="基本信息" name="0">
                                ...
                                <el-form-item label="商品价格" prop="goods_price">
                                    <el-input v-model.number="editForm.counter_price" type="number"></el-input>
                                </el-form-item>
                                <el-form-item label="折扣价格" prop="goods_price">
                                    <el-input v-model.number="editForm.retail_price" type="number"></el-input>
                                </el-form-item>
                                <el-form-item label="会员价格" >
                                    <el-input v-model.number="editForm.vip_price" type="number"></el-input>
                                </el-form-item>
                            </el-tab-pane>
                            <el-tab-pane label="商品属性" name="1">
                                <el-tabs tab-position="left">
                                    <el-form-item>
                                        <span style="float:left;">商品图片：</span>
                                        <el-popover placement="left" trigger="click" width="300" v-show="editForm.pic_url">
                                            <img :src=editForm.pic_url width="100%"/>
                                            <img slot="reference" :src=editForm.pic_url
                                                 style="max-height: 70px;max-width: 70px; padding: 5px"/>
                                        </el-popover>
                                        <input type="file" @change="uploadUrl(1)" accept="image/*"/>
                                    </el-form-item>
                                    <el-form-item>
                                        <span style="float:left;">轮转图片：</span>
<!--                                        <div v-for="item of editForm.gallery" :key="item" v-show="editForm.gallery" style="float: left">-->
<!--                                            <el-popover placement="left" trigger="click" width="300">-->
<!--                                                <img :src=item width="100%" alt="item"/>-->
<!--                                                <img slot="reference" :src=item-->
<!--                                                     style="max-height: 70px;max-width: 70px; padding: 5px" alt="item"/>-->
<!--                                            </el-popover>-->
<!--                                        </div>-->
                                        <el-popover placement="left" trigger="click" width="300"
                                                    v-for="item in main(editForm.gallery)" :key="item">
                                            <img :src=items(item) width="100%" alt="items(item)"/>
                                            <img slot="reference" :src=items(item)
                                                 style="max-height: 70px;max-width: 70px; padding: 5px" alt="items(item)"/>
                                        </el-popover>
                                        <input type="file" @change="uploadUrl(2)" accept="image/*">
                                    </el-form-item>
                                    <el-form-item>
                                        <span style="float:left;">介绍：</span>
                                        <textarea v-model="editForm.detail"
                                                  style="height: 500px;width: 800px;overflow:auto"></textarea>
                                    </el-form-item>
                                </el-tabs>
                            </el-tab-pane>
                            ...
                            <el-tab-pane label="商品参数" name="2">
                                ...
                                <el-form-item v-for="(item, index) in editForm.parameter" :key="index">
                                    参数名称：
                                    <el-input type="text" v-model="item.value"/>
                                    输入价格：
                                    <el-input type="text" v-model="item.price"/>
                                    输入数量：
                                    <el-input type="text" v-model="item.number"/>
                                    <br/>
                                    商品售价：
                                    <el-input type="text" v-model="item.discount_price"/>
                                    折扣价格：
                                    <el-input type="text" v-model="item.integral_price"/>
                                    会员价格：
                                    <el-input type="text" v-model="item.vip_price"/>
                                    <br/>
                                    图片：
                                    <el-popover placement="left" trigger="click" width="300">
                                        <img :src=item.pic_url width="100%" :alt=item.pic_url />
                                        <img slot="reference" :src=item.pic_url
                                             style="max-height: 70px;max-width: 70px; padding: 5px" :alt=item.pic_url />
                                    </el-popover>
                                    <input type="file" @change="uploadImg(index)" accept="image/*"/>
                                </el-form-item>
                                <el-button class="btn-add" @click="onAdd">添加参数</el-button>
                                <el-button class="btn-add" @click="onDel">删除参数</el-button>
