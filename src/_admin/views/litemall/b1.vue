<template>
    <div>
        ...
        <div style="float:right;width: 38%;height:50pc">
            ...
            <el-button @click="save3">生成新文档</el-button>
...
// 监听器
    methods: {
        edit() {
            ...
            html.save2({data: this.html, type: this.show}).then((res) => {
                if (this.show) {
                    console.log("获取模板")
                    console.log(res);
        ...
        newAdd() {
            html.newAdd().then((res) => {
                this.initdate();
            });
        },
        save(data,type) {
            data={...data,type}
            html.save(data).then((res) => {
                this.text = res.data.page.fileContent;
                this.list = res.data.page.list;
                this.$nextTick(() => {
                    const iframe = this.$refs.myIframe;
        ...
        save3: function () {
            html.save3().then((res) => {
                    window.open(resUrl("res/service.html"), "_blank", "width=500,height=400");
                }
            );
