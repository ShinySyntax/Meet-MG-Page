    ...
    <div class="commonright container groupChatMage">
        ...
        <div class="filter">
            <div class="filter-item">
                <label class="filter-label">发起者用户tiono</label>
                <el-input type="text" clearable v-model="filters.searchkey" placeholder="tiono"></el-input>
            </div>
            <div class="filter-item">
                <label class="filter-label">流水号</label>
                <el-input type="text" clearable v-model="filters.walletid" placeholder="流水号"></el-input>
            </div>
        ...
        <div class="contentpad">
            ...
            <el-table-column label="流水号" prop="phone" :align="$protovar.align" width="200">
                    <template slot-scope="scope">
                        {{ scope.row.merorderid }}
                    </template>
                </el-table-column>
                <el-table-column label="发起者用户" :align="$protovar.align" width="220">
                    <template slot-scope="scope">
                        <div class="user">
                            <div>
                                <el-image :src="scope.row.sendAvatar" class="imgcol tmopera">
                            ...
                            <div class="userInfo">
                                <span class="userInfo-nick color_666">{{ scope.row.sendNick }} </span>
                                <span class="color_999">ID:{{ scope.row.sendTiono }} </span>
                                <span class="color_999">手机号:{{ scope.row.sendPhone }} </span>
                            </div>
                        </div>
                    </template>
                </el-table-column>
                <el-table-column label="接收者用户" :align="$protovar.align" width="220">
                    <template slot-scope="scope">
                        <div class="user">
                            <div>
                                <el-image :src="scope.row.toAvatar" class="imgcol tmopera">
                                    <div slot="error" class="image-slot">
                                        <img src="~@_/assets/img/im/avatar.jpg" class="error-img"/>
                                    </div>
                                </el-image>
                            ...
                            <div class="userInfo">
                                <span class="userInfo-nick color_666">{{ scope.row.toNick }} </span>
                                <span class="color_999">ID:{{ scope.row.toTiono }} </span>
                                <span class="color_999">手机号:{{ scope.row.toPhone }} </span>
                            </div>
                        </div>
                    </template>
                </el-table-column>
                <el-table-column label="金额" :align="$protovar.align" width="200">
                    <template slot-scope="scope">
                        <div>
                            <p>￥{{ scope.row.cny / 100 || 0 }}元</p>
                        </div>
                    </template>
                </el-table-column>
...
export default {
    ...
    methods: {
        ...
        getData(item) {
            ...
            console.log(ptdata);
            redPrurse.transferRecordlist(ptdata).then(res => {
                console.log(res)
                if (res.ok) {
                    let data = res.data;
                    if (data) {
                        ...
                        let list = data.list;
                        list.map(item => {
                            item.sendAvatar = resUrl(item.sendAvatar);
